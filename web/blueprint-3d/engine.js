...
var SHEET_CATALOG={
  A000:{title:'Cover, approvals & general notes',discipline:'administrative'},
  SITE:{title:'Grading & drainage plan with civil details',discipline:'civil-site'},
  A101:{title:'Demolition site plan',discipline:'demolition'},
  A102:{title:'New site plan',discipline:'civil-site'},
  A103:{title:'Architectural floor plan',discipline:'architectural'},
  A104:{title:'Reflected ceiling & lighting plan',discipline:'electrical-lighting'},
  A105:{title:'Roof plan',discipline:'architectural-roof'},
  A106:{title:'Exterior elevations',discipline:'architectural-elevation'},
  A107:{title:'Building sections',discipline:'architectural-section'},
  A108:{title:'Plumbing plan, schedule & isometric',discipline:'plumbing'},
  A109:{title:'Mechanical plan',discipline:'mechanical'},
  A110:{title:'Gas plan',discipline:'gas'},
  E001:{title:'Electrical notes, load calculations & panel schedules',discipline:'electrical'},
  E002:{title:'Power plan',discipline:'electrical'},
  S001:{title:'Structural notes & schedules',discipline:'structural'},
  S002:{title:'Foundation plan & details',discipline:'structural-foundation'},
  S003:{title:'Structural details',discipline:'structural'},
  S004:{title:'Framing plan & details',discipline:'structural-framing'}
};
...
function mount(root,initialModel){
  ...
  var model=initialModel,state={stage:0,mode:'orbit',dimensions:true,section:0,level:'first',playing:false,selected:null,mapSync:false,showConcept:true},disciplinePageFilter=null;
  ...
  function saveState(){try{localStorage.setItem(storageKey('view'),JSON.stringify({version:1,stage:state.stage,dimensions:state.dimensions,section:state.section,level:state.level,showConcept:state.showConcept,camera:camera}));}catch(e){}}
  function restoreState(){try{var saved=JSON.parse(localStorage.getItem(storageKey('view'))||'null');if(saved&&saved.version===1){state.stage=Math.max(0,Math.min(STAGES.length-1,saved.stage|0));state.dimensions=saved.dimensions!==false;state.section=Math.max(0,Math.min(3,saved.section|0));state.level=['first','second','roof','all'].indexOf(saved.level)>=0?saved.level:'first';state.showConcept=saved.showConcept!==false;if(saved.camera&&Number.isFinite(saved.camera.dist))camera=saved.camera;}}catch(e){try{localStorage.removeItem(storageKey('view'));}catch(ignore){}}}
  ...
  function compile(){
    ...
    (model.geometry.elements||[]).forEach(function(el){
      if(el.concept&&!state.showConcept)return;
  ...
  function loadModel(next,shouldPersist){
    if(global.AssistifyConceptGeometry)next=global.AssistifyConceptGeometry.augment(next);var errors=validateModel(next);if(errors.length)throw new Error(errors.slice(0,8).join(' '));
  ...
  function roleColor(role,active,alpha){var palette={concrete:'#8ca1b8',steel:'#8fd5ed',wood:'#c69154',envelope:'#438fe8',mep:'#d978ce',finish:'#d7e4ee',earth:'#927251',utility:'#e1a431','concept-site':'#67c587','concept-demo':'#e08181','concept-earth':'#b99260','concept-utility':'#d9a83d','concept-concrete':'#aab9c8','concept-wood':'#e0a15d','concept-roof':'#67a9ef','concept-plumbing':'#53a9ff','concept-mechanical':'#4fd0c8','concept-gas':'#f1c44f','concept-electrical':'#ed72b6'};var conceptual=String(role).indexOf('concept-')===0,hex=conceptual?(palette[role]||'#d2a35d'):(active?'#38b9ff':(palette[role]||'#87a9cd'));return hexToRgba(hex,alpha);}
  ...
  function render(){
    ...
    list.forEach(function(item){var active=item.el.stage===state.stage,selected=state.selected&&state.selected.id===item.el.source.id,concept=!!item.el.source.concept;ctx.setLineDash(concept?[6,4]:[]);ctx.beginPath();ctx.moveTo(item.screen[0][0],item.screen[0][1]);for(var i=1;i<item.screen.length;i++)ctx.lineTo(item.screen[i][0],item.screen[i][1]);if(item.kind==='face'){ctx.closePath();ctx.fillStyle=roleColor(item.el.role,active,concept ? .12 : (selected?.9:(active?.48:.26)));ctx.fill();}ctx.strokeStyle=selected?'#ffe16b':roleColor(item.el.role,active,.95);ctx.lineWidth=selected?3:(active?1.8:1);ctx.stroke();ctx.setLineDash([]);});
  ...
  function displayGeometry(){return(model.geometry.elements||[]).filter(function(el){return state.showConcept||!el.concept;});}
  function renderStage(){var stage=STAGES[state.stage],available=displayGeometry(),atStage=available.filter(function(el){return STAGE_INDEX[el.buildStage]===state.stage;}),visible=available.filter(function(el){return STAGE_INDEX[el.buildStage]<=state.stage;});$('#hudStage').textContent='Stage '+(state.stage+1)+' / '+STAGES.length+' · '+stage.name;$('#hudSelection').textContent=state.selected?state.selected.name:(elements.length?'Select Inspect to view evidence.':'No plan-supported geometry loaded.');$('#stageStatus').textContent=atStage.length?stage.description:'UNVERIFIED — no plan-supported geometry is available for this stage.';$('#visibleCount').textContent=visible.length;$('#stageCount').textContent=atStage.length;$('#modelDescription').textContent=model.project.displayName+', '+model.project.status+'. Stage '+(state.stage+1)+' of '+STAGES.length+', '+stage.name+'. '+visible.length+' supported or explicitly conceptual elements are visible.';}
  function renderInspection(){var box=$('#inspection'),list=$('#sourceList');if(!state.selected){box.textContent='Select Inspect, then choose supported geometry in the viewport.';list.innerHTML='<li class="empty">No source citation selected.</li>';renderStage();return;}var el=state.selected;box.innerHTML='<b>'+esc(el.name)+'</b>'+(el.concept?'<strong>CONCEPTUAL GUESS — NOT COORDINATED GEOMETRY</strong>':'')+esc(el.state)+' · '+esc(el.buildStage)+'<br>Facts: '+esc(el.factRefs.join(', '))+(el.inference?'<br>Basis: '+esc(el.inference):'');list.innerHTML=el.sourceRefs.map(function(ref){return'<li data-source-key="'+esc(sourceKey(ref))+'"><b>'+esc(ref.documentId)+'</b>, page '+ref.page+', region '+esc(ref.region)+(el.concept?'<br><em>Source supports the concept only; coordinates are approximate.</em>':'')+'</li>';}).join('');renderStage();}
  function noteData(){var fact=model.facts&&model.facts.planNoteRegister,value=fact&&fact.value;return{value:value||{},entries:value&&Array.isArray(value.entries)?value.entries:[]};}
  function actionLabel(action){return String(action||'UNCLASSIFIED').replace(/^REGISTERED_/,'').replace(/_/g,' ').toLowerCase().replace(/(^|\s)\S/g,function(c){return c.toUpperCase();});}
  function sheetRows(){var data=noteData(),docs=model.sources.documents||[],pageCount=docs.reduce(function(max,doc){return Math.max(max,doc.pageCount||0);},data.value.coverage&&data.value.coverage.pageCount||0),rows=[];for(var page=1;page<=pageCount;page++)rows.push({page:page,sheet:'Page '+page,title:'Imported plan sheet',discipline:'general',entryIds:{},occurrences:0,actions:{},geometry:0,concept:0});data.entries.forEach(function(entry){(entry.occurrences||[]).forEach(function(occ){var row=rows[occ.page-1];if(!row)return;row.sheet=occ.sheet||row.sheet;var catalog=SHEET_CATALOG[row.sheet];if(catalog){row.title=catalog.title;row.discipline=catalog.discipline;}row.entryIds[entry.id]=true;row.occurrences++;row.actions[entry.modelAction]=true;});});(model.geometry.elements||[]).forEach(function(el){var seen={};(el.sourceRefs||[]).forEach(function(ref){if(rows[ref.page-1]&&!seen[ref.page]){if(el.concept)rows[ref.page-1].concept++;else rows[ref.page-1].geometry++;seen[ref.page]=true;}});});return rows;}
  function disciplineGroup(row){if(row.sheet==='A000')return'administrative';if(row.sheet==='SITE'||/^A10[12]$/.test(row.sheet))return'civil-site';if(/^A10[3-7]$/.test(row.sheet))return'architectural';if(row.sheet==='A108')return'plumbing';if(row.sheet==='A109')return'mechanical';if(row.sheet==='A110')return'gas';if(/^E/.test(row.sheet))return'electrical';if(/^S/.test(row.sheet))return'structural';return row.discipline;}
  function setupNoteFilters(rows,entries){var sheet=$('#noteSheet'),discipline=$('#noteDiscipline'),action=$('#noteAction'),disciplines={},actions={};entries.forEach(function(entry){(entry.disciplines||[]).forEach(function(item){disciplines[item]=true;});actions[entry.modelAction]=true;});sheet.innerHTML='<option value="">All sheets</option>'+rows.map(function(row){return'<option value="'+row.page+'">'+esc(row.sheet)+' — '+esc(row.title)+'</option>';}).join('');discipline.innerHTML='<option value="">All disciplines</option>'+Object.keys(disciplines).sort().map(function(item){return'<option value="'+esc(item)+'">'+esc(item.replace(/-/g,' '))+'</option>';}).join('');action.innerHTML='<option value="">All statuses</option>'+Object.keys(actions).sort().map(function(item){return'<option value="'+esc(item)+'">'+esc(actionLabel(item))+'</option>';}).join('');}
  function renderPlanIntelligence(){var data=noteData(),entries=data.entries,coverage=data.value.coverage||{},rows=sheetRows(),linked=rows.filter(function(row){return row.geometry>0;}).length,conceptSheets=rows.filter(function(row){return row.concept>0;}).length,conceptElements=(model.geometry.elements||[]).filter(function(el){return el.concept;}).length,applied=entries.filter(function(entry){return entry.modelAction==='APPLIED_TO_MODEL';}).length,registered=entries.length-applied;$('#sheetCount').textContent=rows.length?'('+rows.length+')':'';$('#coverageMessage').textContent=entries.length?'All '+rows.length+' sheets have searchable plan intelligence. '+linked+' sheet(s) support the reviewed simplified model; '+conceptSheets+' sheet(s) now contribute explicitly guessed concept geometry. Dashed concepts are not coordinated construction geometry.':'No plan analysis loaded.';$('#intelligenceMetrics').innerHTML=entries.length?'<div class="metric"><b>'+rows.length+'</b><span>Sheets analyzed</span></div><div class="metric"><b>'+entries.length+'</b><span>Unique entries</span></div><div class="metric"><b>'+linked+' / '+rows.length+'</b><span>Reviewed 3D sources</span></div><div class="metric"><b>'+conceptElements+'</b><span>Concept guesses</span></div><div class="metric"><b>'+registered+'</b><span>Registered constraints</span></div><div class="metric"><b>'+((coverage.kindCounts&&coverage.kindCounts['schedule-or-calculation'])||0)+'</b><span>Schedules / calculations</span></div>':'';var groups=[['administrative','Approvals & general'],['civil-site','Civil / site'],['architectural','Architectural'],['plumbing','Plumbing'],['mechanical','Mechanical'],['gas','Gas'],['electrical','Electrical'],['structural','Structural']];$('#disciplineGrid').innerHTML=entries.length?groups.map(function(group){var key=group[0],pages=rows.filter(function(row){return disciplineGroup(row)===key;}).map(function(row){return row.page;}),pageMap={},entryIds={};pages.forEach(function(page){pageMap[page]=true;});entries.forEach(function(entry){if((entry.occurrences||[]).some(function(occ){return pageMap[occ.page];}))entryIds[entry.id]=true;});return pages.length?'<button type="button" class="discipline-card" data-discipline="'+esc(key)+'" data-pages="'+pages.join(',')+'"><b>'+esc(group[1])+'</b><span>'+Object.keys(entryIds).length+' entries · '+pages.length+' sheet(s)</span></button>':'';}).join(''):'';$('#sheetList').innerHTML=rows.length?rows.map(function(row){var count=Object.keys(row.entryIds).length,parts=[];if(row.geometry)parts.push(row.geometry+' reviewed model element(s)');if(row.concept)parts.push(row.concept+' concept guess(es)');if(!parts.length)parts.push('Indexed; geometry unverified');return'<button type="button" class="sheet-card" data-page="'+row.page+'"><b>Page '+row.page+' · '+esc(row.sheet)+' — '+esc(row.title)+'</b><span>'+count+' unique entries · '+row.occurrences+' occurrences · '+esc(row.discipline.replace(/-/g,' '))+'</span><span class="sheet-state'+(row.geometry?'':' indexed')+'">'+esc(parts.join(' · '))+'</span></button>';}).join(''):'<p class="empty">No sheets loaded.</p>';setupNoteFilters(rows,entries);}
  ...
  function renderNoteRegister(){var data=noteData(),entries=data.entries,needle=String($('#noteSearch').value||'').trim().toLowerCase(),page=Number($('#noteSheet').value||0),discipline=$('#noteDiscipline').value,action=$('#noteAction').value,filtered=entries.filter(function(entry){if(page&&!(entry.occurrences||[]).some(function(item){return item.page===page;}))return false;if(disciplinePageFilter&&!(entry.occurrences||[]).some(function(item){return disciplinePageFilter.indexOf(item.page)>=0;}))return false;if(discipline&&!(entry.disciplines||[]).some(function(item){return item===discipline||(discipline==='architectural'&&item.indexOf('architectural')===0)||(discipline==='structural'&&item.indexOf('structural')===0);}))return false;if(action&&entry.modelAction!==action)return false;if(!needle)return true;return[entry.text,entry.kind,entry.modelAction,(entry.disciplines||[]).join(' '),(entry.occurrences||[]).map(function(item){return item.sheet+' page '+item.page;}).join(' ')].join(' ').toLowerCase().indexOf(needle)>=0;}),coverage=data.value.coverage||{};$('#noteCount').textContent=entries.length?'('+entries.length+')':'';$('#noteSummary').textContent=entries.length?((coverage.nonemptyLineOccurrences||0)+' extracted line occurrence(s), '+entries.length+' unique entries across '+(coverage.pageCount||sheetRows().length)+' sheet(s). '+filtered.length+' shown; registered items do not create geometry until reviewed and applied.'):'No note register loaded.';$('#noteList').innerHTML=filtered.length?filtered.map(function(entry){var occurrences=entry.occurrences||[],matched=page?occurrences.filter(function(item){return item.page===page;}):(disciplinePageFilter?occurrences.filter(function(item){return disciplinePageFilter.indexOf(item.page)>=0;}):occurrences),source=matched[0]||entry.firstSource||{},sheets={},sheetText;matched.forEach(function(item){sheets[item.sheet||('Page '+item.page)]=true;});sheetText=Object.keys(sheets).slice(0,4).join(', ');if(Object.keys(sheets).length>4)sheetText+=' +'+(Object.keys(sheets).length-4);return'<li><b>'+esc(entry.text)+'</b><span class="note-meta">'+esc(actionLabel(entry.modelAction))+' · '+esc(entry.kind)+' · '+esc((entry.disciplines||[]).join(', '))+' · '+esc(sheetText||('page '+(source.page||'—')))+(entry.occurrenceCount>1?' · '+entry.occurrenceCount+' occurrences':'')+'</span><br>'+esc(entry.rationale||'')+'</li>';}).join(''):'<li class="empty">No notes or details match these filters.</li>';}
  ...
  function renderOutline(){var list=$('#modelOutline'),byStage={};STAGES.forEach(function(stage){byStage[stage.id]=[];});displayGeometry().forEach(function(el){byStage[el.buildStage].push(el);});list.innerHTML=STAGES.map(function(stage,index){var items=byStage[stage.id];return'<li><b>'+(index+1)+'. '+esc(stage.name)+'</b> — '+(items.length?items.map(function(el){return esc(el.name)+' ('+(el.concept?'CONCEPT GUESS':esc(el.state))+')';}).join('; '):'UNVERIFIED — no supported geometry')+'</li>';}).join('');}
  ...
  function renderUI(){document.getElementById('projectName').textContent=model.project.displayName;document.getElementById('projectStatus').textContent=model.project.status;var notice=$('#modelNotice'),data=noteData(),reviewed=(model.geometry.elements||[]).filter(function(el){return!el.concept;}).length,concept=(model.geometry.elements||[]).filter(function(el){return el.concept;}).length,toggle=$('#toggleConcept');disciplinePageFilter=null;notice.innerHTML=reviewed||concept?'<strong>'+esc(model.project.status)+':</strong> '+reviewed+' reviewed simplified element(s) plus '+concept+' explicitly guessed concept element(s). '+data.entries.length+' note and detail entries remain traceable below.':'<strong>UNVERIFIED GEOMETRY:</strong> '+data.entries.length+' plan entries were indexed, but none have been applied as 3D shapes.';$('#truthBanner').textContent=concept&&state.showConcept?'Dashed, discipline-colored shapes are conceptual guesses. Solid shapes are the earlier simplified model; inspect any element for its source and inference basis.':(reviewed?'Concept guesses are hidden. The remaining model is still simplified and INFERRED, not permit-grade geometry.':'Reference grid only — imported notes do not become geometry without drawing review.');toggle.disabled=!concept;toggle.setAttribute('aria-pressed',String(!!concept&&state.showConcept));toggle.textContent=concept?(state.showConcept?'Hide concept geometry':'Show concept geometry'):'No concept geometry';$('#conceptStatus').textContent=concept?(state.showConcept?concept+' dashed, discipline-colored approximations are visible. They are not permit-, survey-, coordination-, or fabrication-grade geometry.':concept+' concept approximations are hidden; use Show concept geometry to restore them.'):'No conceptual geometry has been generated for this model.';stageSelect.innerHTML=STAGES.map(function(stage,index){return'<option value="'+index+'">'+(index+1)+' — '+esc(stage.name)+'</option>';}).join('');stageSelect.value=String(state.stage);$('#floorLevel').value=state.level;$('#noteSearch').value='';$('#noteSheet').value='';$('#noteDiscipline').value='';$('#noteAction').value='';renderRegister();renderPlanIntelligence();renderNoteRegister();renderUnknowns();renderOutline();renderStage();renderInspection();updateTools();}
  function toggleConcept(){if(!model.geometry.elements.some(function(el){return el.concept;}))return;state.showConcept=!state.showConcept;state.selected=null;elements=compile();computeBounds();saveState();renderUI();fit();announce('Concept geometry '+(state.showConcept?'shown.':'hidden.'));}
  ...
  async function loadBundledModel(button,path,label,announcement){var error=$('#uploadError'),oldText=button.textContent;error.hidden=true;button.disabled=true;button.setAttribute('aria-busy','true');button.textContent='Loading…';try{var response=await fetch(path,{cache:'no-store'});if(!response.ok)throw new Error(label+' could not be downloaded.');var next=await response.json(),errors=validateModel(next);if(errors.length)throw new Error(errors.slice(0,8).join(' '));loadModel(next,true);announce(announcement);}catch(e){error.textContent=label+' load rejected: '+e.message;error.hidden=false;}finally{button.disabled=false;button.removeAttribute('aria-busy');button.textContent=oldText;}}
  function loadSampleModel(){return loadBundledModel($('#loadSampleModel'),'sample-full-floor-plan-model.json','Sample model','Loaded the complete Los Angeles sample floor plan.');}
  function loadApprovedModel(){return loadBundledModel($('#loadApprovedModel'),'approvedplans-4752-25-assistify-model.json','Approved plan','Loaded the 4752-25 approved-plan Assistify model.');}
  async function uploadPdf(event){var file=event.target.files&&event.target.files[0],error=$('#uploadError'),status=$('#pdfStatus'),input=event.target,label=input.closest('label');error.hidden=true;status.textContent='';if(!file)return;input.disabled=true;if(label)label.setAttribute('aria-busy','true');try{if(!global.AssistifyPdf)throw new Error('PDF importer is unavailable.');var result=await global.AssistifyPdf.importFile(file,function(message){status.textContent=message;});var errors=validateModel(result.model);if(errors.length)throw new Error(errors.slice(0,8).join(' '));loadModel(result.model,true);status.textContent=result.summary;announce(result.summary);}catch(e){error.textContent='PDF import rejected: '+e.message;error.hidden=false;status.textContent='';}finally{input.disabled=false;if(label)label.removeAttribute('aria-busy');event.target.value='';}}
  ...
  renderTools();stageSelect.addEventListener('change',function(){setStage(Number(this.value));});$('#floorLevel').addEventListener('change',function(){state.level=this.value;saveState();setDirty();announce(this.options[this.selectedIndex].text+' selected.');});$('#toggleConcept').addEventListener('click',toggleConcept);$('#noteSearch').addEventListener('input',renderNoteRegister);$('#noteSheet').addEventListener('change',function(){disciplinePageFilter=null;renderNoteRegister();});$('#noteDiscipline').addEventListener('change',function(){disciplinePageFilter=null;renderNoteRegister();});$('#noteAction').addEventListener('change',renderNoteRegister);$('#disciplineGrid').addEventListener('click',function(event){var button=event.target.closest('[data-discipline]');if(!button)return;$('#noteSheet').value='';$('#noteDiscipline').value=button.dataset.discipline;disciplinePageFilter=button.dataset.pages.split(',').map(Number);renderNoteRegister();$('#noteRegisterDetails').scrollIntoView({block:'start',behavior:reduceMotion?'auto':'smooth'});});$('#sheetList').addEventListener('click',function(event){var button=event.target.closest('[data-page]');if(!button)return;disciplinePageFilter=null;$('#noteSheet').value=button.dataset.page;renderNoteRegister();$('#noteRegisterDetails').scrollIntoView({block:'start',behavior:reduceMotion?'auto':'smooth'});});$('#stageBack').addEventListener('click',function(){stepStage(-1);});$('#stageForward').addEventListener('click',function(){stepStage(1);});$('#loadApprovedModel').addEventListener('click',loadApprovedModel);$('#loadSampleModel').addEventListener('click',loadSampleModel);$('#pdfUpload').addEventListener('change',uploadPdf);$('#modelUpload').addEventListener('change',upload);$('#exportModel').addEventListener('click',downloadModel);$('#clearModel').addEventListener('click',clearModel);
