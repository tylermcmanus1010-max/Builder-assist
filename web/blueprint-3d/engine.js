...
function mount(root,initialModel){
  ...
  var model=initialModel,state={stage:0,mode:'orbit',dimensions:true,section:0,playing:false,selected:null,mapSync:false};
  ...
  function saveState(){try{localStorage.setItem(storageKey('view'),JSON.stringify({version:1,stage:state.stage,dimensions:state.dimensions,section:state.section,camera:camera}));}catch(e){}}
  function restoreState(){try{var saved=JSON.parse(localStorage.getItem(storageKey('view'))||'null');if(saved&&saved.version===1){state.stage=Math.max(0,Math.min(STAGES.length-1,saved.stage|0));state.dimensions=saved.dimensions!==false;state.section=Math.max(0,Math.min(2,saved.section|0));if(saved.camera&&Number.isFinite(saved.camera.dist))camera=saved.camera;}}catch(e){try{localStorage.removeItem(storageKey('view'));}catch(ignore){}}}
  ...
  function sectionVisible(el){if(!state.section)return true;var axis=state.section===1?0:1,cut=bounds.center[axis];return el.points.some(function(p){return p[axis]<=cut;});}
  ...
  function drawGrid(w,h){ctx.save();ctx.lineWidth=1;for(var i=-5;i<=5;i++){drawWorldLine([i*10,-50,0],[i*10,50,0],i===0?'rgba(104,212,255,.55)':'rgba(110,145,181,.18)',w,h);drawWorldLine([-50,i*10,0],[50,i*10,0],i===0?'rgba(104,212,255,.55)':'rgba(110,145,181,.18)',w,h);}ctx.restore();}
  ...
  function activateTool(id){
    ...
    if(id==='section'){state.section=(state.section+1)%3;saveState();updateTools();setDirty();announce(state.section===0?'Section view off.':(state.section===1?'X section active.':'Y section active.'));return;}
  ...
  function updateTools(){TOOLS.forEach(function(tool){var button=dock.querySelector('[data-tool="'+tool.id+'"]'),reason=toolDisabledReason(tool.id),pressed=(['orbit','pan','zoom','measure','inspect'].indexOf(tool.id)>=0&&state.mode===tool.id)||(tool.id==='sequence'&&state.playing)||(tool.id==='dimensions'&&state.dimensions)||(tool.id==='section'&&state.section>0)||(tool.id==='map-sync'&&state.mapSync);button.textContent=tool.label+(tool.id==='section'&&state.section?' '+(state.section===1?'X':'Y'):'');button.disabled=!!reason;button.title=reason||tool.label+' ('+tool.shortcut+')';button.setAttribute('aria-label',reason?tool.label+' unavailable: '+reason:tool.label+', shortcut '+tool.shortcut);button.setAttribute('aria-pressed',String(!!pressed));});}
  ...
  async function loadSampleModel(){var error=$('#uploadError');error.hidden=true;try{var response=await fetch('sample-residential-site-plan-model.json',{cache:'no-store'});if(!response.ok)throw new Error('Sample model could not be downloaded.');var next=await response.json(),errors=validateModel(next);if(errors.length)throw new Error(errors.slice(0,8).join(' '));loadModel(next,true);announce('Loaded the Minneapolis sample residential site plan.');}catch(e){error.textContent='Sample load rejected: '+e.message;error.hidden=false;}}
