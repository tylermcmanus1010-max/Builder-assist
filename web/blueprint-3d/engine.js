...
function mount(root,initialModel){
  ...
  var model=initialModel,state={stage:0,mode:'orbit',dimensions:true,section:0,level:'first',playing:false,selected:null,mapSync:false},disciplinePageFilter=null;
  ...
  function sheetRows(){var data=noteData(),docs=model.sources.documents||[],pageCount=docs.reduce(function(max,doc){return Math.max(max,doc.pageCount||0);},data.value.coverage&&data.value.coverage.pageCount||0),rows=[];for(var page=1;page<=pageCount;page++)rows.push({page:page,sheet:'Page '+page,title:'Imported plan sheet',discipline:'general',entryIds:{},occurrences:0,actions:{},geometry:0});data.entries.forEach(function(entry){(entry.occurrences||[]).forEach(function(occ){var row=rows[occ.page-1];if(!row)return;row.sheet=occ.sheet||row.sheet;var catalog=SHEET_CATALOG[row.sheet];if(catalog){row.title=catalog.title;row.discipline=catalog.discipline;}row.entryIds[entry.id]=true;row.occurrences++;row.actions[entry.modelAction]=true;});});(model.geometry.elements||[]).forEach(function(el){var seen={};(el.sourceRefs||[]).forEach(function(ref){if(rows[ref.page-1]&&!seen[ref.page]){rows[ref.page-1].geometry++;seen[ref.page]=true;}});});return rows;}
  ...
  function renderUI(){document.getElementById('projectName').textContent=model.project.displayName;document.getElementById('projectStatus').textContent=model.project.status;var notice=$('#modelNotice'),data=noteData(),rows=sheetRows(),linked=rows.filter(function(row){return row.geometry>0;}).length;disciplinePageFilter=null;notice.innerHTML=model.geometry.elements.length?'<strong>'+esc(model.project.status)+':</strong> '+model.geometry.elements.length+' cited geometry element(s) from '+linked+' of '+rows.length+' analyzed sheet(s). '+data.entries.length+' note and detail entries remain traceable below.':'<strong>UNVERIFIED GEOMETRY:</strong> '+data.entries.length+' plan entries were indexed, but none have been applied as 3D shapes.';$('#truthBanner').textContent=model.geometry.elements.length?'3D shows only reviewed, cited geometry. Use Plan intelligence for civil, structural, MEP, schedules, calculations, notes, and details not yet reconstructed.':'Reference grid only — imported notes do not become geometry without drawing review.';stageSelect.innerHTML=STAGES.map(function(stage,index){return'<option value="'+index+'">'+(index+1)+' — '+esc(stage.name)+'</option>';}).join('');stageSelect.value=String(state.stage);$('#floorLevel').value=state.level;$('#noteSearch').value='';$('#noteSheet').value='';$('#noteDiscipline').value='';$('#noteAction').value='';renderRegister();renderPlanIntelligence();renderNoteRegister();renderUnknowns();renderOutline();renderStage();renderInspection();updateTools();}
