...
The main site embeds this shared viewer in an iframe on the admin
`#/admin-portal/assistify` tab and as station 02 of the contractor member
portal. Because the site also ships as a single published artifact -- where no
sibling file is reachable -- `tools/build-assistify-bundle.mjs` emits a
self-contained one-file build of this directory to `web/assistify-bundle/`, and
the site falls back to it through `srcdoc` when the sibling app does not load.
Run
...
after changing anything here; the first regenerates the bundle, the second
refreshes the embedded payload and the mount contract. The admin
`#/admin-portal/model` tab is a different tool -- the Van Horn seven-stage
construction-sequence viewer in `web/vanhorn-3d/` -- generated by
`web/main-site/build-vanhorn3d.py`.
