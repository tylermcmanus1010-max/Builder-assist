...
export function reconcileBlueprintIR(modelValue: ProjectModel, irValue: BlueprintIR): ProjectModel {
  ...
  for (const geometry of ir.geometryCandidates) {
    ...
    const scale = ir.scaleCandidates.find((candidate) => candidate.viewportId === geometry.viewportId && candidate.status === "dimension_verified");
    if (!sheet || !scale || next.geometry2D.some((candidate) => candidate.geometryId === geometry.geometryId)) continue;
    const points = geometry.points.map((point) => ({ x: point.x * scale.ratio / 864, y: point.y * scale.ratio / 864 })) as [{ x: number; y: number }, { x: number; y: number }];
    const length = Math.hypot(points[1].x - points[0].x, points[1].y - points[0].y);
    const thickness = geometry.thicknessPdfPoints * scale.ratio / 864;
    ...
    else assumptions.push("Wall height was not found; Three.js geometry remains unavailable.");
