...
try{
  ...
  await page.locator('#loadSampleModel').click();await page.waitForFunction(()=>document.querySelector('#projectName').textContent.includes('Minneapolis'));
  ...
  check(sample.geometry===5&&sample.status==='INFERRED'&&sample.docs===1&&sample.unknown===6&&!sample.error,'bundled Minneapolis sample model loads with source-linked 2D geometry and explicit unknowns',JSON.stringify(sample));
