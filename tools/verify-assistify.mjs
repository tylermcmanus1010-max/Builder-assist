...
try{
  ...
  if(process.env.APPROVED_PDF_PATH){await page.locator('#pdfUpload').setInputFiles(process.env.APPROVED_PDF_PATH);await page.waitForFunction(()=>assistifyViewer.getModel().project.id==='scottsdale-4752-25-approvedplans');const approvedPdf=await page.evaluate(()=>({geometry:assistifyViewer.getModel().geometry.elements.length,notes:assistifyViewer.getModel().facts.planNoteRegister.value.entries.length,status:document.querySelector('#pdfStatus').textContent,error:document.querySelector('#uploadError').hidden?'':document.querySelector('#uploadError').textContent}));check(approvedPdf.geometry===30&&approvedPdf.notes===1547&&/Recognized the reviewed ApprovedPlans PDF/.test(approvedPdf.status)&&!approvedPdf.error,'exact ApprovedPlans PDF restores the reviewed geometry and note model',JSON.stringify(approvedPdf));}
